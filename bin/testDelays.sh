...
if [ -s "$pubDelayFile" ]; then
    ...
    if [ "$pubDelayHours" -ne "0" ]; then
        ...
        if [ -n "$pubDelayWaitFullDays" ]; then
            # test if delay hours has passed strictly.
            if [ -s "$pubDelayFile" ] && [ "$(($(date +"%s") - $(stat -c "%Y" "$pubDelayFile")))" -lt "$pubDelaySeconds" ]; then
                if [ -z "$forceRun" ]; then
                    echo "Published file is not yet $(($pubDelayHours / 24)) days old - $(stat -c '%y' "$pubDelayFile" | cut -d: -f1,2)"
                    exit 0
                fi
            fi
        else
