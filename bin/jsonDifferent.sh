...
#
# if file1 is missign or empty, return same if file2 is missing or empty, otherwise, they are differnt.
if [ ! -s "$1" ]; then
    if [ ! -s "$2" ]; then
        exit 1
    fi
...
# we have to non-zero files, compare them using the node app.
