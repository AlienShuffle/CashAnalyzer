...
json.sort(dynamicSort('ticker', 'asOfDate'));
let result = [];
let last = { 'ticker': '' };
...
for (let i = 0; i < json.length; i++) {
    ...
    if (newDate.getTime() == lastDate.getTime()) {
        ...
        if (
            ...
            (thisPriority == lastPriority &&   // if same priority, use one with more precision or one-day yields reported.
                (
                    (json[i].sevenDayYield.length > last.sevenDayYield.length) ||
