...
function sourcePriority(source) {
    ...
    if (src.indexOf('blackrock') > -1) return 9;
    if (src.indexOf('ssga') > -1) return 9;
    if (src.indexOf('alphaarchitect') > -1) return 8;
    // 6 is the break for getting rid of items that on weekends to allow the higher values to be included.
    // this is our default baseline, assume funds are more complete (don't trust on weekends).
    if (src.indexOf('edgar') > -1) return 5;
    // all items below are considered less reliable, and will be overridden.
    if (src.indexOf('deep') > -1) return 4;         // markit.com as data source.
    if (src.indexOf('cnbc') > -1) return 4;
...
for (let i = 0; i < json.length; i++) {
    ...
    if (thisPriority == 1
        || (thisPriority < 5 &&
            (thisDay == 0 ||
                thisDay == 6 ||                     // skip SAT/SUN if from untrusted source
                (thisMonth == 11 && thisDate == 31) // skip New Year's Eve if from untrusted source
            )
        )
    ) {
        continue;
    }
    switch (json[i].ticker) {
        case 'BASOX':
        case 'BBSOX':
        ...
        if (
            ...
            (thisPriority == lastPriority &&   // if same priority, use one with more precision or one-day yields, or 30-days reported.
                (
                    ...
                    // when all else equal, keep the newest timestamp, empties get replaced.
                    (jsonTime > lastTime)
                )
            )
        ) {
...
// use the sortBest (run all the time!) to rationalize object names.
// on 2/18/26 added to rename price to nav, this can be used for other keys in the future...
result.forEach(obj => {
    if (obj.hasOwnProperty("price")) {
        obj.nav = obj.price; // copy value to new key
        delete obj.price;    // remove old key
    }
});
