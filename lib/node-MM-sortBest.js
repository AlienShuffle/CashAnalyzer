...
for (let i = 0; i < json.length; i++) {
    ...
    if (thisPriority == 1 ||
        (thisPriority < 6 && (thisDay == 0 || thisDay == 6)) ||   // skip SAT/SUN
        (thisMonth == 11 && thisDate == 31)                       // skip New Year's Eve
    ) {
        continue;
    }
        ...
        if (
            ...
            (thisPriority == lastPriority &&   // if same priority, use one with more precision or one-day yields, or 30-days reported.
                (
                    ...
                    // when all else equal, keep the oldest timestamp, empties get replaced.
                    (jsonTime > 0 && (lastTime == 0 || jsonTime < lastTime))
                )
            )
        ) {
            result.pop();
