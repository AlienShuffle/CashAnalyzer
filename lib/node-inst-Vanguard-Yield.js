...
for (let i = 0; i < json.length; i++) {
    ...
    for (let j = 0; j < numRows; j++) {
        ...
        switch (yield.yieldCode) {
            case '1DISTYLD':
                ...
                break;
            case 'SEC':
                if (safeObjectRef(yield.methodCode) && yield.methodCode.startsWith('SEC-30')) {
                    //console.error(`Found ${yield.methodCode} yield code, mapping to thirtyDayYield`);
    ...
    for (const effectiveDate of Object.keys(buffer)) {
        ...
        // only assume nav of 1 if sevenday yield exists (we are MoneyMarket).
        if (safeObjectRef(buffer[effectiveDate].sevenDayYield)) tickerData.nav = 1;
