...
csvHistoryFlare="$cloudFlareHome/Treasuries/$bankName/$bankName-history.csv"
...
if [ -f "$jsonHistoryFlare" ]; then
    jq -s 'flatten | unique_by([.cusip,.asOfDate]) | sort_by([.cusip,.asOfDate])' "$jsonRateNew" "$jsonHistoryFlare" >tmp-flatten.json
...
if [ $lenHistoryUnique -gt $lenHistoryFlare ]; then
    ...
    (
       echo 'asOfDate, cusip, securitytype, rate, maturitydate, calldate, buy, sell, endofdate'
        jq -r '.[] | [.asOfDate, .cusip, .securitytype, .rate, .maturitydate, .calldate, .buy, .sell, .endofdate] | @csv' "$jsonHistoryFlare"
    ) >"$csvHistoryFlare"
